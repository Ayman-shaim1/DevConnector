...
export const createPost = (text) => {
  return async (dispatch, getState) => {
    try {
      ...
      const { data } = await axios.post("/api/posts", dataToSend, config);
...
export const resetListPost = () => {
  return { type: POST_LIST_RESET };
};
...
export const deleteCommentFromPost = (idPost, idComment) => {
  return async (dispatch, getState) => {
    try {
      ...
      dispatch({ type: POST_GET_BY_ID_UPDATE_DELETE_COMMENT, payload: data });
      dispatch({
